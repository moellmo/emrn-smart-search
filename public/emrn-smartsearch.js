(() => {
  ...
  style.textContent = `
    ...
    .emrn-smart-filter-toggle{display:none;width:100%;height:44px;margin:0 0 14px;border:1px solid #c34d50;background:#fff;color:#c34d50;border-radius:999px;font-weight:900;align-items:center;justify-content:center;cursor:pointer}.emrn-smart-results-shell{display:grid;grid-template-columns:280px 1fr;gap:22px}
  ...
  function trackSmartSearchEvent(event,detail={}){try{const payload={event,query:String(detail.query||currentQuery()||"").trim(),sku:detail.sku||"",product_name:detail.product_name||"",product_id:Number(detail.product_id||0)||0,customer_id:config.customerId?String(config.customerId):"",page_type:pageType||"",url:window.location.href};const body=JSON.stringify(payload);if(navigator.sendBeacon){navigator.sendBeacon(`${config.apiBase}/api/search-analytics`,new Blob([body],{type:"application/json"}));return}fetch(`${config.apiBase}/api/search-analytics`,{method:"POST",mode:"cors",keepalive:true,headers:{"Content-Type":"application/json"},body}).catch(()=>{})}catch{}}
  function goToResults(term){const q=String(term||activeInput?.value||"").trim();if(q.length<2)return;navigatingToSmartResults=true;suppressOverlayUntilClick=true;try{sessionStorage.setItem(SUPPRESS_KEY,"1")}catch{}clearTimeout(renderTimer);saveRecentSearch(q);trackSmartSearchEvent("search",{query:q});hideOverlay();document.querySelectorAll(".emrn-smartsearch-overlay").forEach((el)=>el.remove());try{activeInput?.blur()}catch{}const url=new URL(config.searchResultsUrl,window.location.origin);url.searchParams.set("search_query",q);url.searchParams.set(config.resultsParam,"1");window.location.href=url.toString()}
  ...
  function sideCard(title,items){if(!items||!items.length)return"";const chipType=/brand/i.test(title)||/marque/i.test(title)?"brand":/categor/i.test(title)||/catégor/i.test(title)?"category":"search";return `<div class="emrn-smartsearch-card"><h4>${escapeHtml(title)}</h4><div class="emrn-smartsearch-chips">${items.slice(0,7).map((item)=>{const value=typeof item==="string"?item:item.value;const count=typeof item==="string"?"":` <small>(${item.count})</small>`;const categoryUrl=typeof item==="string"?"":(item.url||"");const categoryId=typeof item==="string"?"":(item.id||categoryByName(value)?.id||"");return `<button type="button" class="emrn-smartsearch-chip" data-emrn-search="${escapeHtml(value)}" data-emrn-chip-type="${chipType}" ${categoryId?`data-emrn-category-id="${escapeHtml(categoryId)}"`:""} ${categoryUrl?`data-emrn-category-url="${escapeHtml(categoryUrl)}"`:""}>${escapeHtml(value)}${count}</button>`}).join("")}</div></div>`}
  ...
  function ensureOverlay(){if(overlay&&document.body.contains(overlay))return overlay;document.querySelectorAll(".emrn-smartsearch-overlay").forEach((el)=>el.remove());overlay=document.createElement("div");overlay.className="emrn-smartsearch-overlay";overlay.hidden=true;overlay.style.display="none";document.body.appendChild(overlay);overlay.addEventListener("mousedown",(e)=>e.preventDefault());overlay.addEventListener("click",(e)=>{const productLink=e.target.closest("a.emrn-smartsearch-item");if(productLink){hideOverlay();return}const viewAll=e.target.closest("[data-emrn-viewall]");if(viewAll){e.preventDefault();e.stopPropagation();goToResults(activeInput?.value||viewAll.getAttribute("data-emrn-viewall")||"");return}const chip=e.target.closest("[data-emrn-search]");if(chip&&activeInput){e.preventDefault();e.stopPropagation();const term=chip.getAttribute("data-emrn-search")||"";const chipType=chip.getAttribute("data-emrn-chip-type")||"search";if(chipType==="category"){const id=chip.getAttribute("data-emrn-category-id")||"";if(id){goToCategoryId(id);return}const categoryUrl=chip.getAttribute("data-emrn-category-url")||"";if(categoryUrl){hideOverlay();window.location.href=categoryUrl;return}}if(chipType==="brand"){hideOverlay();goToFilteredResults("brand",term);return}activeInput.value=term;saveRecentSearch(term);lastRendered="";renderResults(term);activeInput.focus()}},true);return overlay}
  ...
  async function renderResults(query){if(navigatingToSmartResults)return;ensureOverlay();if(!overlay)return;if(!query||query.trim().length<2){renderStarter();return}const seq=++requestSeq;lastRendered=query;overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div><div class="emrn-smartsearch-loading">Searching...</div></div><div class="emrn-smartsearch-side"><div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay();try{const params=appendCustomerParam(new URLSearchParams({q:query}));const res=await fetch(`${config.apiBase}/api/autocomplete?${params.toString()}`,{mode:"cors"});const data=await res.json();if(seq!==requestSeq||navigatingToSmartResults)return;const products=data.products||[];const brandFacet=(data.facets||[]).find((facet)=>facet.field==="brand");const categoryFacet=(data.facets||[]).find((facet)=>facet.field==="categories");overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div>${products.length?products.map(productItem).join(""):renderNoSuggestions(query,data)}${products.length?`<button type="button" class="emrn-smartsearch-viewall" data-emrn-viewall="${escapeHtml(query)}">${t("viewAll")} “${escapeHtml(query)}”</button>`:""}</div><div class="emrn-smartsearch-side">${sideCard(t("suggestedCategories"),categoryFacet?.values||[])}${sideCard(t("suggestedBrands"),brandFacet?.values||[])}${sideCard(t("popular"),POPULAR_SEARCHES)}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay()}catch(err){if(seq!==requestSeq||navigatingToSmartResults)return;console.error("[EMRN SmartSearch] API error",err);overlay.innerHTML=`<div class="emrn-smartsearch-empty">SmartSearch could not load right now. Please try again.</div>`;showOverlay()}}
  async function renderStarter(){if(navigatingToSmartResults)return;ensureOverlay();if(!overlay)return;const recent=getRecentSearches().slice(0,4);const popular=POPULAR_SEARCHES.slice(0,6);overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("popularProducts")}</div><div class="emrn-smartsearch-loading">Loading popular products...</div></div><div class="emrn-smartsearch-side">${sideCard(t("categories"),starterSidebarCategories())}${sideCard(t("popular"),popular)}${recent.length?sideCard(t("recent"),recent):""}</div></div>`;showOverlay();try{const params=new URLSearchParams();params.set("per_page","12");if(config.customerId)params.set("customer_id",String(config.customerId));const viewed=getViewedSkus().slice(0,12);if(viewed.length)params.set("viewed_skus",viewed.join(","));const [treeResult,popularResult]=await Promise.allSettled([loadCategoryTree(),fetch(`${config.apiBase}/api/popular?${params.toString()}`,{mode:"cors"}).then(res=>res.json())]);const products=(popularResult.status==="fulfilled"?(popularResult.value.products||[]):[]).slice(0,10);if(!overlay||overlay.hidden||String(activeInput?.value||"").trim().length>=2||navigatingToSmartResults)return;overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("popularProducts")}</div>${products.length?products.map(productItem).join(""):`<div class="emrn-smartsearch-starter"><h3>${t("startTyping")}</h3><p>${t("searchHelp")}</p></div>`}</div><div class="emrn-smartsearch-side">${sideCard(t("categories"),starterSidebarCategories())}${sideCard(t("popular"),popular)}${recent.length?sideCard(t("recent"),recent):""}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay()}catch(err){console.error("[EMRN SmartSearch] starter content error",err)}}
  ...
  function preserveListingFilters(url,except=""){const params=new URLSearchParams(window.location.search);["brand","category","category_id","sold_by","color","price_min","price_max"].forEach(key=>{if(key!==except&&params.get(key))url.searchParams.set(key,params.get(key))})}
  ...
  function currentSearchParamsForApi(page=1){
    ...
    return {apiParams,q,brand,category,categoryId,soldBy,color,priceMin,priceMax}
  }
  ...
  async function renderStorefrontResults(){
    ...
    ensureBackTopButton();
    const {q,brand,category,categoryId,soldBy,color,priceMin,priceMax}=currentSearchParamsForApi(1);
