(() => {
  ...
  style.textContent = `
    ...
    .emrn-smart-results-page{width:min(1560px,calc(100% - 40px));max-width:1560px;margin:24px auto 70px;padding:0 20px;box-sizing:border-box;font-family:Arial,sans-serif;color:#1f2937;overflow:hidden}
    ...
    .emrn-smart-products-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;align-items:stretch;min-width:0}
  ...
  function correctionText(value){
    ...
    if(typeof value==="object"){
      return correctionText(value.suggested_query||value.query||value.normalized_query||value.value||value.label||value.text||value.corrected);
    }
  ...
  function correctionNotice(data,q,mode="results"){
    const suggested=correctionText(data?.suggested_query)||correctionText(data?.natural_language_plan?.suggested_query)||correctionText(data?.natural_language_plan)||correctionText(data?.correction)||correctionText(data?.query);
