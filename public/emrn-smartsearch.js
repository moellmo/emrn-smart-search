(() => {
  ...
  function categoryCatalogCount(categoryName){const matches=categoriesByName(categoryName);if(!matches.length)return null;return matches.reduce((sum,cat)=>sum+Number(cat.product_count||cat.count||0),0)}
  function categoryForFacet(itemOrName){const value=typeof itemOrName==="string"?itemOrName:itemOrName?.value;const target=Number(typeof itemOrName==="string"?0:itemOrName?.count||0);const matches=categoriesByName(value);if(!matches.length)return null;if(matches.length===1||!target)return matches[0];return matches.slice().sort((a,b)=>{const ac=Number(a.product_count||a.count||0);const bc=Number(b.product_count||b.count||0);const aExact=ac===target?1:0;const bExact=bc===target?1:0;if(aExact!==bExact)return bExact-aExact;const aOver=ac>=target?1:0;const bOver=bc>=target?1:0;if(aOver!==bOver)return bOver-aOver;const aDiff=Math.abs(ac-target);const bDiff=Math.abs(bc-target);if(aDiff!==bDiff)return aDiff-bDiff;return bc-ac})[0]}
