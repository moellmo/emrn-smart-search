(() => {
  ...
  function facetGroup(title,items,type){if(!items||!items.length)return"";const current=currentValueForFilter(type);return `<div class="emrn-smart-filter-group"><h3>${escapeHtml(title)}</h3><div class="emrn-smart-facet-list">${items.slice(0,20).filter(item=>item&&item.value).map((item)=>{const active=String(item.value)===String(current);const category=type==="category"?categoryByName(item.value):null;const attrs=category?.id?`data-full-category-id="${escapeHtml(category.id)}" ${category?.url?`data-category-url="${escapeHtml(normalizeUrl(category.url))}"`:""}`:(category?.url?`data-category-url="${escapeHtml(normalizeUrl(category.url))}"`:`data-filter-type="${type}" data-filter-value="${escapeHtml(item.value)}"`);return `<button type="button" class="emrn-smart-facet ${active?"active":""}" ${attrs}><span class="emrn-smart-facet-name">${escapeHtml(item.value)}</span><span class="emrn-smart-facet-count">${item.count}</span></button>`}).join("")}</div></div>`}
