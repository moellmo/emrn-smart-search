(() => {
  ...
  const config = {
    ...
    // For sandbox set this to the sandbox search.php.
    // For live set this to https://emrn.ca/search.php
  ...
  console.log("[EMRN SmartSearch] storefront results script loaded");
  ...
  style.textContent = `
    ...
    .emrn-smartsearch-name { font-size:14px; font-weight:900; line-height:1.3; color:#1f2937; }
    ...
    .emrn-smart-results-header {
      background: linear-gradient(135deg,#fff 0%,#fff8f8 100%);
      border: 1px solid #f0dada;
      border-radius: 22px;
      padding: 24px;
      margin-bottom: 24px;
      box-shadow: 0 12px 30px rgba(20,30,55,.06);
    }
    .emrn-smart-results-header .eyebrow {
      color:#c34d50;
      font-size:12px;
      font-weight:900;
    ...
    .emrn-smart-results-header h1 {
      margin: 0 0 8px;
      font-size: 32px;
      letter-spacing: -.6px;
    }
    .emrn-smart-results-search {
      display:flex;
      gap:10px;
      margin-top:18px;
    }
    .emrn-smart-results-search input {
      flex:1;
      height:52px;
    ...
    .emrn-smart-results-search button {
      border:0;
      background:#c34d50;
      color:#fff;
      border-radius:999px;
    ...
    .emrn-smart-results-shell {
      display:grid;
      grid-template-columns:260px 1fr;
      gap:26px;
    }
    .emrn-smart-results-filters {
      background:#fff;
    ...
    .emrn-smart-products-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:20px; }
    .emrn-smart-product-card {
      background:#fff;
      border:1px solid #f0dada;
      border-radius:18px;
      overflow:hidden;
    ...
    .emrn-smart-product-img { height:220px; display:flex; align-items:center; justify-content:center; padding:22px; border-bottom:1px solid #f1eeee; text-decoration:none; background:#fff; }
    .emrn-smart-product-img img { max-width:100%; max-height:100%; object-fit:contain; }
    .emrn-smart-product-body { padding:18px; }
    .emrn-smart-stars { color:#999; letter-spacing:2px; font-size:15px; margin-bottom:12px; }
    .emrn-smart-product-name { display:block; min-height:58px; color:#252735; text-decoration:none; font-size:16px; font-weight:900; line-height:1.36; }
    .emrn-smart-product-name:hover { color:#c34d50; }
    .emrn-smart-meta { display:flex; gap:8px; flex-wrap:wrap; margin:10px 0 8px; }
    .emrn-smart-meta span { background:#f3f4f6; border-radius:999px; padding:6px 8px; color:#555; font-size:11px; font-weight:800; }
    .emrn-smart-availability { color:#087a52; font-size:12px; font-weight:900; margin:6px 0 14px; min-height:16px; }
    .emrn-smart-price { font-size:24px; font-weight:400; color:#252735; min-height:32px; margin-bottom:14px; }
    .emrn-smart-qty { height:42px; border:1px solid #f0dada; background:#fffafa; border-radius:999px; display:grid; grid-template-columns:42px 1fr 42px; align-items:center; text-align:center; margin-bottom:10px; overflow:hidden; }
    ...
    .emrn-smart-cart-btn,.emrn-smart-quote-btn,.emrn-smart-view-btn {
      width:100%;
      height:46px;
      border-radius:999px;
      font-weight:900;
    ...
    @media (max-width: 900px) {
      .emrn-smart-results-shell { grid-template-columns:1fr; }
      .emrn-smart-results-filters { position:static; }
      .emrn-smart-products-grid { grid-template-columns:repeat(2,minmax(0,1fr)); }
    }
    @media (max-width: 760px) {
      .emrn-smartsearch-overlay { left:10px!important; right:10px!important; top:84px!important; width:auto!important; max-height:calc(100vh - 100px); overflow:auto; }
      .emrn-smartsearch-grid { grid-template-columns:1fr; }
      .emrn-smartsearch-products { border-right:0; border-bottom:1px solid #eee; }
      .emrn-smartsearch-item { grid-template-columns:54px 1fr; }
      .emrn-smartsearch-view { display:none; }
      .emrn-smart-products-grid { grid-template-columns:1fr; }
      .emrn-smart-results-search { flex-direction:column; }
      .emrn-smart-results-search button { height:48px; }
    }
  `;
  ...
  function productItem(product) {
    ...
    const img = product.image ? `<img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}">` : `<span>No image</span>`;
    return `
      <a class="emrn-smartsearch-item" href="${escapeHtml(url)}">
        ...
        <div>
          ...
          <div class="emrn-smartsearch-meta">${escapeHtml(product.brand || "EMRN")}${product.sku ? ` • SKU: ${escapeHtml(product.sku)}` : ""}</div>
        </div>
  ...
  function ensureOverlay() {
    ...
    overlay.addEventListener("click", (e) => {
      ...
      if (e.target.closest("[data-emrn-viewall]") && activeInput) goToResults(activeInput.value);
    }, true);
    return overlay;
  }
  ...
  function positionOverlay() {
    ...
    const width = Math.min(620, viewportWidth - 24);

    // Anchor to search input left, then clamp. This feels less weird while scrolled/devtools open.
    let left = rect.left;
  ...
  function showOverlay() { ensureOverlay(); positionOverlay(); overlay.hidden = false; overlay.style.display = "block"; document.body.classList.add("emrn-smartsearch-active"); }
  ...
  async function renderResults(query) {
    ensureOverlay();
    if (!query || query.trim().length < 2) { renderStarter(); return; }
    lastRendered = query;
    overlay.innerHTML = `<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">Products</div><div class="emrn-smartsearch-loading">Searching...</div></div><div class="emrn-smartsearch-side"><div class="emrn-smartsearch-help">Can’t find the item?<strong>Request a quote and EMRN can help source it.</strong></div></div></div>`;
    showOverlay();

    try {
      const res = await fetch(`${config.apiBase}/api/autocomplete?q=${encodeURIComponent(query)}`, { mode: "cors" });
      ...
      overlay.innerHTML = `
        <div class="emrn-smartsearch-grid">
          <div class="emrn-smartsearch-products">
            ...
            ${products.length ? products.map(productItem).join("") : `<div class="emrn-smartsearch-empty">No product suggestions found. Try another keyword, SKU, brand, or category.</div>`}
            ${products.length ? `<button type="button" class="emrn-smartsearch-viewall" data-emrn-viewall="1">View all results for “${escapeHtml(query)}”</button>` : ""}
          </div>
          <div class="emrn-smartsearch-side">
            ...
            <div class="emrn-smartsearch-help">Can’t find the item?<strong>Request a quote and EMRN can help source it.</strong></div>
          </div>
        </div>
      `;
      showOverlay();
  ...
  function attach(input) {
    ...
    ensureOverlay();
    input.addEventListener("focus", () => { activeInput = input; scheduleRender(true); }, true);
    input.addEventListener("input", () => { activeInput = input; scheduleRender(true); }, true);
    input.addEventListener("keyup", () => { activeInput = input; scheduleRender(true); }, true);
  }

  async function addToCart(productId, qty, button) {
    ...
    try {
      ...
      const cartsRes = await fetch("/api/storefront/carts", { credentials: "include" });
      const carts = await cartsRes.json();
      const existingCart = Array.isArray(carts) && carts.length ? carts[0] : null;

      const endpoint = existingCart?.id
        ? `/api/storefront/carts/${existingCart.id}/items`
      ...
      document.dispatchEvent(new CustomEvent("emrn-smartsearch-cart-added"));

      // Try common cart drawer triggers. If none exist, this still adds to cart.
      setTimeout(() => {
        const cartTrigger = document.querySelector('[data-cart-preview], .cart-preview, .navUser-action[href*="cart"], a[href="/cart.php"]');
        if (cartTrigger) cartTrigger.click();
      }, 250);
    } catch (err) {
      console.error("[EMRN SmartSearch] add to cart failed", err);
      button.textContent = "Choose Options";
      button.dataset.fallback = "1";
    } finally {
      setTimeout(() => {
        button.disabled = false;
        if (button.textContent === "Added ✓") button.textContent = original;
      }, 1800);
    }
  }
  ...
  function productCard(product) {
    ...
    return `
      <article class="emrn-smart-product-card" data-product-id="${product.product_id}">
        <a class="emrn-smart-product-img" href="${escapeHtml(url)}">${img}</a>
        <div class="emrn-smart-product-body">
          <div class="emrn-smart-stars">★ ★ ★ ★ ★</div>
          <a class="emrn-smart-product-name" href="${escapeHtml(url)}">${escapeHtml(product.name)}</a>
          <div class="emrn-smart-meta">
            ${product.brand ? `<span>${escapeHtml(product.brand)}</span>` : ""}
            ${product.sku ? `<span>SKU: ${escapeHtml(product.sku)}</span>` : ""}
          </div>
          <div class="emrn-smart-availability">${escapeHtml(product.availability_description || product.availability || "Available to order")}</div>
          <div class="emrn-smart-price">${price > 0 ? `$${price.toFixed(2)}` : "See product"}</div>
          <div class="emrn-smart-qty">
            <button type="button" data-qty-minus>−</button>
            <span data-qty>1</span>
            <button type="button" data-qty-plus>+</button>
          </div>
          <button type="button" class="emrn-smart-cart-btn" data-add-cart="${product.product_id}">Add to Cart</button>
          <a class="emrn-smart-view-btn" href="${escapeHtml(url)}">View Product</a>
          <button type="button" class="emrn-smart-quote-btn" data-quote="${product.product_id}">Add to quote</button>
        </div>
      </article>
    `;
  }

  function facetGroup(title, items, type) {
    if (!items || !items.length) return "";
    return `
      <div class="emrn-smart-filter-group">
        <h3>${escapeHtml(title)}</h3>
        <div class="emrn-smart-facet-list">
          ${items.slice(0, 20).map((item) => `
            <button type="button" class="emrn-smart-facet" data-filter-type="${type}" data-filter-value="${escapeHtml(item.value)}">
              <span class="emrn-smart-facet-name">${escapeHtml(item.value)}</span>
              <span class="emrn-smart-facet-count">${item.count}</span>
            </button>
          `).join("")}
        </div>
      </div>
  ...
  async function renderStorefrontResults() {
    ...
    const existing = document.querySelector(".emrn-smart-results-page");
    if (existing) existing.remove();

    const shell = document.createElement("section");
    shell.className = "emrn-smart-results-page";
    shell.innerHTML = `
      <div class="emrn-smart-results-header">
        <div class="eyebrow">EMRN SmartSearch</div>
        <h1>Search results for “${escapeHtml(q)}”</h1>
        <p>Smarter product matching by name, SKU, brand, and category.</p>
        <div class="emrn-smart-results-search">
          <input value="${escapeHtml(q)}" placeholder="Search products, brands, categories, or SKUs..." data-smart-results-input>
          <button type="button" data-smart-results-search>Search</button>
        </div>
      </div>
      <div class="emrn-smart-message">Loading SmartSearch results...</div>
    `;
    ...
    try {
      ...
      const categoryFacet = (data.facet_counts || []).find((facet) => facet.field_name === "categories")?.counts || [];

      shell.innerHTML = `
        <div class="emrn-smart-results-header">
          <div class="eyebrow">EMRN SmartSearch</div>
          <h1>Search results for “${escapeHtml(q)}”</h1>
          <p>${products.length} products shown${brand ? ` • ${escapeHtml(brand)}` : ""}${category ? ` • ${escapeHtml(category)}` : ""}</p>
          <div class="emrn-smart-results-search">
            <input value="${escapeHtml(q)}" placeholder="Search products, brands, categories, or SKUs..." data-smart-results-input>
            <button type="button" data-smart-results-search>Search</button>
          </div>
        </div>
        <div class="emrn-smart-results-shell">
          <aside class="emrn-smart-results-filters">
            <div class="emrn-smart-filter-title">Refine by</div>
            <div class="emrn-smart-filter-note">${brand || category ? "Filters applied" : "Choose a brand or category"}</div>
            ${brand || category ? `<button type="button" class="emrn-smart-viewall" data-clear-filters>Clear filters</button>` : ""}
            ${facetGroup("Brands", brandFacet, "brand")}
            ${facetGroup("Categories", categoryFacet, "category")}
          </aside>
          <div class="emrn-smart-results-main">
            <div class="emrn-smart-results-top">
              <div>
                <h2>Products</h2>
                <p>${products.length} products shown</p>
              </div>
            </div>
            ${products.length ? `<div class="emrn-smart-products-grid">${products.map(productCard).join("")}</div>` : `<div class="emrn-smart-message">No products found. Try another search or request a quote.</div>`}
          </div>
        </div>
      `;
    } catch (err) {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") return hideOverlay();
    if (e.key === "Enter") {
  ...
  document.addEventListener("click", (e) => {
    const searchButton = e.target.closest("[data-smart-results-search]");
    if (searchButton) {
      const input = document.querySelector("[data-smart-results-input]");
      const q = input?.value || "";
      goToResults(q);
      return;
    }
    ...
    if (filter) {
      ...
      url.searchParams.set("search_query", q);
      url.searchParams.set(config.resultsParam, "1");
      url.searchParams.set(filter.dataset.filterType, filter.dataset.filterValue);
      window.location.href = url.toString();
      return;
    }
    ...
    if (plus || minus) {
      ...
      qtyEl.textContent = String(Math.max(1, current + (plus ? 1 : -1)));
      return;
    }
    ...
    if (add) {
      ...
      if (add.dataset.fallback === "1") {
        const view = card.querySelector(".emrn-smart-view-btn");
        if (view) window.location.href = view.href;
        return;
      }
      addToCart(add.dataset.addCart, qty, add);
      return;
    }

    const quote = e.target.closest("[data-quote]");
    if (quote) {
      const card = quote.closest("[data-product-id]");
      const view = card?.querySelector(".emrn-smart-view-btn");
      alert("Quote connection is the next step. For now, open the product page and use Add to Quote.");
      if (view) window.location.href = view.href;
      return;
    }

    if (overlay && !overlay.contains(e.target) && activeInput && e.target !== activeInput) hideOverlay();
  }, true);
  ...
  function init() {
    ...
    if (isResultsPage || params.get("search_query")) {
      // Only replace the page when our param is present. This avoids affecting normal BC search unless intended.
      if (isResultsPage) renderStorefrontResults();
    }
  }
