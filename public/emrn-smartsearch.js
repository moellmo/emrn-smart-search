(() => {
  ...
  console.log("[EMRN SmartSearch] French precision overlay cleanup loaded");
  ...
  function positionOverlay(){if(!activeInput||!overlay)return;const rect=activeInput.getBoundingClientRect();const viewportWidth=window.innerWidth||document.documentElement.clientWidth;const width=Math.min(820,viewportWidth-24);let left=rect.left;if(left+width>viewportWidth-12)left=viewportWidth-width-12;if(left<12)left=12;overlay.style.width=`${width}px`;overlay.style.left=`${left}px`;overlay.style.top=`${rect.bottom+10}px`}
  ...
  function renderStarter(){if(isResultsPage){document.querySelectorAll('.emrn-smartsearch-overlay').forEach((el)=>el.remove());return}ensureOverlay();if(!overlay)return;const recent=getRecentSearches().slice(0,4);const popular=POPULAR_SEARCHES.slice(0,6);overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div><div class="emrn-smartsearch-starter"><h3>${t("startTyping")}</h3><p>${t("searchHelp")}</p>${sideCard(t("popular"),popular)}</div></div><div class="emrn-smartsearch-side">${recent.length?sideCard(t("recent"),recent):sideCard(t("popular"),popular.slice(0,4))}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay()}
