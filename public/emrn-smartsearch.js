(() => {
  ...
  async function renderResults(query){if(navigatingToSmartResults)return;ensureOverlay();if(!overlay)return;if(!query||query.trim().length<2){renderStarter();return}const seq=++requestSeq;lastRendered=query;overlay.innerHTML=`${mobileSearchBox(query)}<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div><div class="emrn-smartsearch-loading">Searching...</div></div><div class="emrn-smartsearch-side"><div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay();restoreMobileFocus(query);try{const params=appendCustomerParam(new URLSearchParams({q:query}));const res=await fetch(`${config.apiBase}/api/autocomplete?${params.toString()}`,{mode:"cors"});const data=await res.json();if(seq!==requestSeq||navigatingToSmartResults)return;const products=data.products||[];const brandFacet=(data.facets||[]).find((facet)=>facet.field==="brand");const categoryFacet=(data.facets||[]).find((facet)=>facet.field==="categories");overlay.innerHTML=`${mobileSearchBox(query)}<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div>${products.length?products.map(productItem).join(""):renderNoSuggestions(query,data)}${products.length?`<button type="button" class="emrn-smartsearch-viewall" data-emrn-viewall="${escapeHtml(query)}">${t("viewAll")} “${escapeHtml(query)}”</button>`:""}</div><div class="emrn-smartsearch-side">${sideCard(t("suggestedCategories"),categoryFacet?.values||[])}${sideCard(t("suggestedBrands"),brandFacet?.values||[])}${sideCard(t("popular"),POPULAR_SEARCHES)}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay();restoreMobileFocus(query)}catch(err){if(seq!==requestSeq||navigatingToSmartResults)return;console.error("[EMRN SmartSearch] API error",err);overlay.innerHTML=`${mobileSearchBox(query)}<div class="emrn-smartsearch-empty">SmartSearch could not load right now. Please try again.</div>`;showOverlay();restoreMobileFocus(query)}}
  ...
  document.addEventListener("keydown",(e)=>{if(isMobileOverlayInput(e.target)){if(e.key==="Escape")return hideOverlay();if(e.key==="Enter"){const q=String(e.target.value||"").trim();if(q.length>=2){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();syncMobileQuery(q);goToResults(q)}}return}if(!isSearchInput(e.target))return;activeInput=e.target;if(e.key==="Escape")return hideOverlay();if(e.key==="Enter"){const q=activeInput.value.trim();if(q.length>=2){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();goToResults(q)}}},true)
