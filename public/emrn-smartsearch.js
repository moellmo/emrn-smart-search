(() => {
  ...
  function categoriesByName(categoryName){const lower=String(categoryName||"").toLowerCase();return (categoryTreeCache?.cats||[]).filter((cat)=>String(cat.name||"").toLowerCase()===lower)}
  function categoryCatalogBranchCount(cat,seen=new Set()){const id=Number(cat?.id||0);if(!id||seen.has(id))return 0;seen.add(id);const own=Number(cat.product_count||cat.count||0);const childTotal=(categoryTreeCache?.byParent.get(id)||[]).reduce((sum,child)=>sum+categoryCatalogBranchCount(child,seen),0);return own+childTotal}
  function categoryCatalogCount(categoryName){const matches=categoriesByName(categoryName);if(!matches.length)return null;return matches.reduce((sum,cat)=>sum+categoryCatalogBranchCount(cat),0)}
  function categoryForFacet(itemOrName){const value=typeof itemOrName==="string"?itemOrName:itemOrName?.value;const target=Number(typeof itemOrName==="string"?0:itemOrName?.count||0);const matches=categoriesByName(value);if(!matches.length)return null;if(matches.length===1||!target)return matches[0];return matches.slice().sort((a,b)=>{const ac=categoryCatalogBranchCount(a);const bc=categoryCatalogBranchCount(b);const aExact=ac===target?1:0;const bExact=bc===target?1:0;if(aExact!==bExact)return bExact-aExact;const aOver=ac>=target?1:0;const bOver=bc>=target?1:0;if(aOver!==bOver)return bOver-aOver;const aDiff=Math.abs(ac-target);const bDiff=Math.abs(bc-target);if(aDiff!==bDiff)return aDiff-bDiff;return bc-ac})[0]}
  function sideCard(title,items){if(!items||!items.length)return"";const chipType=/brand/i.test(title)||/marque/i.test(title)?"brand":/categor/i.test(title)||/catégor/i.test(title)?"category":"search";return `<div class="emrn-smartsearch-card"><h4>${escapeHtml(title)}</h4><div class="emrn-smartsearch-chips">${items.slice(0,7).map((item)=>{const value=typeof item==="string"?item:item.value;const displayCount=chipType==="category"?categoryCatalogCount(value):null;const rawCount=displayCount===null?(typeof item==="string"?"":item.count):displayCount;const count=rawCount!==""&&rawCount!==null&&rawCount!==undefined?` <small>(${rawCount})</small>`:"";const category=chipType==="category"?categoryForFacet(item):null;const categoryUrl=typeof item==="string"?(category?.url||""):(item.url||category?.url||"");const categoryId=typeof item==="string"?(category?.id||""):(item.id||category?.id||"");return `<button type="button" class="emrn-smartsearch-chip" data-emrn-search="${escapeHtml(value)}" data-emrn-chip-type="${chipType}" ${categoryId?`data-emrn-category-id="${escapeHtml(categoryId)}"`:""} ${categoryUrl?`data-emrn-category-url="${escapeHtml(normalizeUrl(categoryUrl))}"`:""}>${escapeHtml(value)}${count}</button>`}).join("")}</div></div>`}
  ...
  function facetGroup(title,items,type){if(!items||!items.length)return"";const current=currentValueForFilter(type);return `<div class="emrn-smart-filter-group"><h3>${escapeHtml(title)}</h3><div class="emrn-smart-facet-list">${items.slice(0,20).filter(item=>item&&item.value).map((item)=>{const active=String(item.value)===String(current);const category=type==="category"?categoryForFacet(item):null;const attrs=type==="category"&&category?.id?`data-category-id="${escapeHtml(category.id)}" ${category?.url?`data-category-url="${escapeHtml(normalizeUrl(category.url))}"`:""}`:`data-filter-type="${type}" data-filter-value="${escapeHtml(item.value)}"`;return `<button type="button" class="emrn-smart-facet ${active?"active":""}" ${attrs}><span class="emrn-smart-facet-name">${escapeHtml(item.value)}</span><span class="emrn-smart-facet-count">${item.count}</span></button>`}).join("")}</div></div>`}
  ...
  function categoryByName(categoryName){return categoriesByName(categoryName)[0]||null}
  function categoryBubbleImage(itemOrName){const category=categoryForFacet(itemOrName)||categoryByName(typeof itemOrName==="string"?itemOrName:itemOrName?.value);return category?.image||""}
  function relatedCategoryBubbles(categoryFacet,products,selectedCategory=""){const items=(categoryFacet||[]).filter((item)=>item&&item.value&&String(item.value).toLowerCase()!==String(selectedCategory||"").toLowerCase()).slice(0,8);if(!items.length)return"";return `<div class="emrn-smart-related-cats" aria-label="${escapeHtml(t("categories"))}">${items.map((item)=>{const image=categoryBubbleImage(item);const category=categoryForFacet(item);const attrs=category?.id?`data-full-category-id="${escapeHtml(category.id)}" ${category?.url?`data-category-url="${escapeHtml(normalizeUrl(category.url))}"`:""}`:(category?.url?`data-category-url="${escapeHtml(normalizeUrl(category.url))}"`:`data-filter-type="category" data-filter-value="${escapeHtml(item.value)}"`);return `<button type="button" class="emrn-smart-related-cat" ${attrs}>${image?`<span class="emrn-smart-related-cat-img"><img src="${escapeHtml(image)}" alt=""></span>`:`<span class="emrn-smart-related-cat-img"></span>`}<span>${escapeHtml(item.value)}</span></button>`}).join("")}</div>`}
  ...
  function currentSearchParamsForApi(page=1){
    ...
    const q=shouldReplaceCategory||shouldReplaceBrand?"*":pageParams.get("search_query")||pageParams.get("q")||"";
