(() => {
  ...
  console.log("[EMRN SmartSearch] SKU-level enabled-only script loaded");
  ...
  style.textContent = `
    ...
    .emrn-smartsearch-option{color:#c34d50;font-size:12px;font-weight:900;margin-top:3px}
    ...
    .emrn-smart-results-filters{background:#fff;border:1px solid #e8e8e8;border-radius:18px;padding:18px;height:max-content;position:sticky;top:20px}
    ...
    .emrn-smart-product-card{background:#fff;border:1px solid #f0dada;border-radius:18px;overflow:hidden;box-shadow:0 8px 22px rgba(0,0,0,.04);transition:all .16s ease}
    ...
    .emrn-smart-variant{color:#c34d50;font-size:13px;font-weight:900;margin:8px 0}
    ...
    .emrn-smart-price{font-size:24px;font-weight:400;color:#252735;min-height:32px;margin:14px 0}
    ...
    .emrn-smart-cart-btn,.emrn-smart-quote-btn,.emrn-smart-view-btn{width:100%;height:46px;border-radius:999px;font-weight:900;cursor:pointer;transition:all .14s ease;margin-top:8px}
    ...
    @media (max-width:900px){.emrn-smart-results-shell{grid-template-columns:1fr}.emrn-smart-results-filters{position:static}.emrn-smart-products-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
  ...
  function sideCard(title,items){if(!items||!items.length)return"";return `<div class="emrn-smartsearch-card"><h4>${escapeHtml(title)}</h4><div class="emrn-smartsearch-chips">${items.slice(0,7).map((item)=>{const value=typeof item==="string"?item:item.value;const count=typeof item==="string"?"":` <small>(${item.count})</small>`;return `<button type="button" class="emrn-smartsearch-chip" data-emrn-search="${escapeHtml(value)}">${escapeHtml(value)}${count}</button>`}).join("")}</div></div>`}
  function productItem(product){const url=normalizeUrl(product.url);const img=product.image?`<img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}">`:`<span>No image</span>`;return `<a class="emrn-smartsearch-item" href="${escapeHtml(url)}"><div class="emrn-smartsearch-img">${img}</div><div><div class="emrn-smartsearch-name">${escapeHtml(product.parent_name||product.name)}</div>${product.option_text?`<div class="emrn-smartsearch-option">${escapeHtml(product.option_text)}</div>`:""}<div class="emrn-smartsearch-meta">${escapeHtml(product.brand||"EMRN")}${product.sku?` • SKU: ${escapeHtml(product.sku)}`:""}</div></div><div class="emrn-smartsearch-view">View</div></a>`}

  function ensureOverlay(){if(overlay&&document.body.contains(overlay))return overlay;document.querySelectorAll(".emrn-smartsearch-overlay").forEach((el)=>el.remove());overlay=document.createElement("div");overlay.className="emrn-smartsearch-overlay";overlay.hidden=true;overlay.style.display="none";document.body.appendChild(overlay);overlay.addEventListener("mousedown",(e)=>e.preventDefault());overlay.addEventListener("click",(e)=>{const chip=e.target.closest("[data-emrn-search]");if(chip&&activeInput){const term=chip.getAttribute("data-emrn-search")||"";activeInput.value=term;saveRecentSearch(term);lastRendered="";renderResults(term);activeInput.focus()}if(e.target.closest("[data-emrn-viewall]")&&activeInput)goToResults(activeInput.value)},true);return overlay}
  ...
  function productCard(product){const url=normalizeUrl(product.url);const price=Number(product.price||0);const img=product.image?`<img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}">`:`<span>No image</span>`;return `<article class="emrn-smart-product-card" data-product='${escapeHtml(JSON.stringify({product_id:product.product_id,variant_id:product.variant_id||0,url}))}'><a class="emrn-smart-product-img" href="${escapeHtml(url)}">${img}</a><div class="emrn-smart-product-body"><div class="emrn-smart-stars">★ ★ ★ ★ ★</div><a class="emrn-smart-product-name" href="${escapeHtml(url)}">${escapeHtml(product.parent_name||product.name)}</a>${product.option_text?`<div class="emrn-smart-variant">${escapeHtml(product.option_text)}</div>`:""}<div class="emrn-smart-meta">${product.brand?`<span>${escapeHtml(product.brand)}</span>`:""}${product.sku?`<span>SKU: ${escapeHtml(product.sku)}</span>`:""}</div><div class="emrn-smart-price">${price>0?`$${price.toFixed(2)}`:"See product"}</div><div class="emrn-smart-qty"><button type="button" data-qty-minus>−</button><span data-qty>1</span><button type="button" data-qty-plus>+</button></div><button type="button" class="emrn-smart-cart-btn" data-add-cart>Add to Cart</button><a class="emrn-smart-view-btn" href="${escapeHtml(url)}">View Product</a><button type="button" class="emrn-smart-quote-btn" data-quote>Add to quote</button></div></article>`}
  ...
  setInterval(()=>{const input=getInput();if(input&&input!==activeInput)attach(input);if(activeInput){const current=activeInput.value||"";if(current!==lastValue){lastValue=current;scheduleRender(true)}}},300)
  function init(){const input=getInput();if(input)attach(input);if(isResultsPage)renderStorefrontResults()}
