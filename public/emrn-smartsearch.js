(() => {
  ...
  function goToFullCategoryName(name){const clean=String(name||"").trim();if(!clean)return;const url=new URL(config.searchResultsUrl,window.location.origin);url.searchParams.set("search_query","*");url.searchParams.set(config.resultsParam,"1");url.searchParams.set("category",clean);window.location.href=url.toString()}
