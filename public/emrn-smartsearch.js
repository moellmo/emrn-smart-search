(() => {
  ...
  console.log("[EMRN SmartSearch] French no-results category tree loaded");
  ...
  const COPY = {
    en: {
      products: "Products", popular: "Popular searches", recent: "Recent searches",
      ...
      trySearches: "Try these searches", requestQuote: "Request a quote", subcategories: "Subcategories",
    },
    fr: {
      products: "Produits", popular: "Recherches populaires", recent: "Recherches récentes",
      ...
      trySearches: "Essayez ces recherches", requestQuote: "Demander un devis", subcategories: "Sous-catégories",
    }
  };
  ...
  const POPULAR_SEARCHES = LANG === "fr"
    ? ["gants", "masques", "oxygène", "pansement", "seringue", "tensiomètre", "RCR", "premiers soins"]
    : ["gloves", "masks", "AED", "oxygen", "foley catheter", "wound dressing", "stethoscope", "CPR manikin"];
  ...
  function renderNoSuggestions(query,data){const terms=data?.fallback_terms||POPULAR_SEARCHES.slice(0,5);return `<div class="emrn-smartsearch-empty"><strong>${t("noResultsTitle")}</strong><br>${t("noResultsBody")}<div class="emrn-smartsearch-chips" style="margin-top:10px">${terms.map(term=>`<button type="button" class="emrn-smartsearch-chip" data-emrn-search="${escapeHtml(term)}">${escapeHtml(term)}</button>`).join("")}</div></div>`}

  async function renderResults(query){if(isResultsPage)return;ensureOverlay();if(!query||query.trim().length<2){renderStarter();return}lastRendered=query;overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div><div class="emrn-smartsearch-loading">Searching...</div></div><div class="emrn-smartsearch-side"><div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay();try{const res=await fetch(`${config.apiBase}/api/autocomplete?q=${encodeURIComponent(query)}`,{mode:"cors"});const data=await res.json();const products=data.products||[];const brandFacet=(data.facets||[]).find((facet)=>facet.field==="brand");const categoryFacet=(data.facets||[]).find((facet)=>facet.field==="categories");overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div>${products.length?products.map(productItem).join(""):renderNoSuggestions(query,data)}${products.length?`<button type="button" class="emrn-smartsearch-viewall" data-emrn-viewall="1">${t("viewAll")} “${escapeHtml(query)}”</button>`:""}</div><div class="emrn-smartsearch-side">${sideCard(t("suggestedBrands"),brandFacet?.values||[])}${sideCard(t("suggestedCategories"),categoryFacet?.values||[])}${sideCard(t("popular"),POPULAR_SEARCHES)}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay()}catch(err){console.error("[EMRN SmartSearch] API error",err);overlay.innerHTML=`<div class="emrn-smartsearch-empty">SmartSearch could not load right now. Please try again.</div>`;showOverlay()}}
  function renderStarter(){if(isResultsPage)return;ensureOverlay();const recent=getRecentSearches();overlay.innerHTML=`<div class="emrn-smartsearch-grid"><div class="emrn-smartsearch-products"><div class="emrn-smartsearch-title">${t("products")}</div><div class="emrn-smartsearch-starter"><h3>${t("startTyping")}</h3><p>${t("searchHelp")}</p>${sideCard(t("popular"),POPULAR_SEARCHES)}${recent.length?sideCard(t("recent"),recent):""}</div></div><div class="emrn-smartsearch-side">${sideCard(t("popular"),POPULAR_SEARCHES)}${recent.length?sideCard(t("recent"),recent):""}<div class="emrn-smartsearch-help">${t("cantFind")}<strong>${t("sourceHelp")}</strong></div></div></div>`;showOverlay()}
  function scheduleRender(force=false){if(isResultsPage||!activeInput)return;const value=activeInput.value.trim();if(!force&&value===lastRendered)return;clearTimeout(renderTimer);renderTimer=setTimeout(()=>renderResults(value),120)}
  ...
  function renderCategoryBranch(parentId,level=0,selectedId=0){if(!categoryTreeCache)return"";const children=(categoryTreeCache.byParent.get(Number(parentId))||[]).sort((a,b)=>a.name.localeCompare(b.name));return children.map(cat=>{const hasChildren=(categoryTreeCache.byParent.get(Number(cat.id))||[]).length>0;const isActive=Number(selectedId)===Number(cat.id);return `<div class="emrn-smart-cat-node" data-cat-node="${cat.id}" style="margin-left:${level?10:0}px"><div class="emrn-smart-cat-row ${isActive?"active":""}">${hasChildren?`<button type="button" class="emrn-smart-cat-toggle" data-cat-toggle="${cat.id}">+</button>`:`<span style="width:24px"></span>`}<button type="button" class="emrn-smart-cat-link" data-category-id="${cat.id}" title="${escapeHtml(cat.name)}">${escapeHtml(cat.name)}</button></div>${hasChildren?`<div class="emrn-smart-cat-children" data-cat-children="${cat.id}">${renderCategoryBranch(cat.id,level+1,selectedId)}</div>`:""}</div>`}).join("")}
  function renderCategoryTree(selectedId){return `<div class="emrn-smart-filter-group"><h3>${t("categories")}</h3><div class="emrn-smart-category-tree">${renderCategoryBranch(0,0,selectedId)}</div></div>`}
  ...
  async function renderStorefrontResults(){hideOverlay();document.querySelectorAll('.emrn-smartsearch-overlay').forEach((el)=>el.remove());const pageParams=new URLSearchParams(window.location.search);const q=pageParams.get("search_query")||pageParams.get("q")||"";const brand=pageParams.get("brand")||"";const category=pageParams.get("category")||"";const categoryId=Number(pageParams.get("category_id")||0);const main=document.querySelector("main")||document.querySelector(".body")||document.body;document.querySelector(".emrn-smart-results-page")?.remove();const shell=document.createElement("section");shell.className="emrn-smart-results-page";shell.innerHTML=`<div class="emrn-smart-results-header"><div class="eyebrow">EMRN SmartSearch</div><h1>${t("resultsFor")} “${escapeHtml(q)}”</h1><p>${t("smarterMatching")}</p><div class="emrn-smart-results-search"><input value="${escapeHtml(q)}" placeholder="${t("searchPlaceholder")}" data-smart-results-input><button type="button" data-smart-results-search>${t("searchButton")}</button></div></div><div class="emrn-smart-message">Loading SmartSearch results...</div>`;main.prepend(shell);await loadCategoryTree();try{const apiParams=new URLSearchParams();apiParams.set("q",q||"*");if(brand)apiParams.set("brand",brand);if(category)apiParams.set("category",category);if(categoryId)apiParams.set("category_id",String(categoryId));const res=await fetch(`${config.apiBase}/api/search?${apiParams.toString()}`,{mode:"cors"});const data=await res.json();const products=(data.hits||[]).map((hit)=>hit.document);const brandFacet=(data.facet_counts||[]).find((facet)=>facet.field_name==="brand")?.counts||[];shell.innerHTML=`<div class="emrn-smart-results-header"><div class="eyebrow">EMRN SmartSearch</div><h1>${t("resultsFor")} “${escapeHtml(q)}”</h1><p>${products.length} ${t("skuLevelShown")}${brand?` • ${escapeHtml(brand)}`:""}${category?` • ${escapeHtml(category)}`:""}</p><div class="emrn-smart-results-search"><input value="${escapeHtml(q)}" placeholder="${t("searchPlaceholder")}" data-smart-results-input><button type="button" data-smart-results-search>${t("searchButton")}</button></div></div><div class="emrn-smart-results-shell"><aside class="emrn-smart-results-filters"><div class="emrn-smart-filter-title">${t("refineBy")}</div><div class="emrn-smart-filter-note">${brand||category||categoryId?t("filtersApplied"):t("chooseBrandCategory")}</div>${brand||category||categoryId?`<button type="button" class="emrn-smart-viewall" data-clear-filters>${t("clearFilters")}</button>`:""}${facetGroup(t("brands"),brandFacet,"brand")}${renderCategoryTree(categoryId)}</aside><div class="emrn-smart-results-main"><div class="emrn-smart-results-top"><div><h2>${t("products")}</h2><p>${products.length} ${t("skuLevelShown")}</p></div></div>${products.length?`<div class="emrn-smart-products-grid">${products.map(productCard).join("")}</div>`:noResultsBox(data,q)}</div></div>`}catch(err){console.error("[EMRN SmartSearch] results page error",err);shell.innerHTML=`<div class="emrn-smart-message">SmartSearch results could not load. Please try again.</div>`}}
